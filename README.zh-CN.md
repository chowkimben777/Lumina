...
4. 点击“媒体”，查看当前媒体状态。
5. Codex 或 Trae 在后台完成工具任务或较长响应时，Lumina 会弹出 3 秒完成提醒；点击提醒卡可回到对应应用。
6. 鼠标离开面板后，岛体会自动收起；右键点击岛体可退出应用。
...
Lumina 不会上传剪贴板内容，也会忽略常见密码管理器使用的隐藏剪贴板类型。AI 完成提醒只读取本机 Codex 会话事件与 Trae Agent 活动，不会上传任务内容或活动数据。QQ 音乐通过 macOS 26 的系统媒体桥接读取；由于 Apple 没有公开的跨应用媒体元数据 API，这项支持属于实验性能力。
